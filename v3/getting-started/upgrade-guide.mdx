...
Inertia no longer ships with or requires Axios. A built-in XHR client is used instead. If your application relies on Axios interceptors for Inertia requests, you may provide a custom HTTP client. See the [client-side setup](/v3/installation/client-side-setup) documentation for more details.
